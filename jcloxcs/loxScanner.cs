    ...
    class Scanner
    {
        ...
        private static TokenType checkKeyword(int start, int length, string rest, TokenType type)
        {
            if (scanner.current - scanner.start == start + length &&
               util.util._memcmp(scanner._char_ptr, scanner.start + start, rest, length))
            {
                return type;
            }
        ...
        // Taken from jlox
        public static List<Token> scanTokens(char[] source)
        {
            initScanner(source);
            ...
            while (!isAtEnd())
            {
                ...
                // start = current; already inside scanToken
                tokens.Add(scanToken());
            }

            //probably causing REPL to fail 
            tokens.Add(makeToken(TokenType.TOKEN_EOF));
            return tokens;
        }
